<template>
  <div class="text-scroll pointer-events-auto h-full w-full overflow-hidden">
    ...
    >
      ...
      <div v-if="config.isOverflow && !disableAnimate">
        <slot></slot>
      </div>
    </div>
  </div>
...
<script setup>
import { onMounted, ref, reactive, useSlots, watch, nextTick, watchEffect } from 'vue'
...
const opts = {
  initAnimate: () => {
    if (props.disableAnimate) return
  ...
  init: () => {
    ...
    opts.initAnimate()
    if (animation && props.disableAnimate) {
